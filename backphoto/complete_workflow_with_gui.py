...
# -*- coding: utf-8 -*-
"""
Complete Workflow with GUI Email Input
Detects head and automatically sends email with captured photo
Includes graphical email input interface
"""
...
from email_input_gui import get_email_input
from config import EMAIL_CONFIG

class CompleteWorkflowWithGUI:
    def __init__(self):
    ...
    def get_email_input_gui(self):
        """使用图形界面获取邮箱输入"""
        print("\n📧 Opening email input window...")
        try:
            ...
            print(f"❌ Error getting email input: {str(e)}")
            return False
    
    def start_detection_with_email(self):
        """开始检测并自动发送邮件"""
        print("=" * 60)
        print("🎭 The Unseen Portrait - Complete Workflow with GUI")
        print("=" * 60)
        print("This system will:")
        print("1. Open email input window")
        print("2. Detect the back of your head")
        print("3. Take a photo automatically")
        print("4. Send the photo via email")
        
        # 获取邮箱输入
        if not self.get_email_input_gui():
...
def main():
    """Main function"""
    print("🎭 The Unseen Portrait - Complete System with GUI")
    print("This system will automatically:")
    print("• Open email input window")
    ...
    workflow = CompleteWorkflowWithGUI()
